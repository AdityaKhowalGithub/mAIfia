...
# polly_client = boto3.client('polly', region_name='us-east-1')  # Replace with your region
...
#     game = games[game_id]
#     # Process actions performed by roles during the night
#     # For example, Mafia selects a player to eliminate

#     # For simplicity, assume Mafia action is stored in game state
...
#                 # You can add logic to check if all night actions are completed
#                 # and then proceed to the day phase
    ...
    if 'AudioStream' in response:
        ...
        if audio_stream:
            return send_file(
                ...
                mimetype='audio/mpeg',
                as_attachment=False,
                download_name='speech.mp3'  # Updated here
            )
        else:
